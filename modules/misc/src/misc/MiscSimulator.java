...
public class MiscSimulator extends StandardSimulator implements GUIComponent {
    ...
    protected void processCommands(KSCommands c, ChangeSet changes) {
            ...
            if (checkValidity(com)) {
                ...
                * */
                if (com instanceof AKUnload) {
                    ...
                    if (target != null) {
                        Entity AgentPosition = model.getEntity(((Human) agent).getPosition());
        ...
        int wl = 0;
        for(Map.Entry r: waitingList.entrySet())
        {
            wl = ((Deque<EntityID>)r.getValue()).size();
            Logger.warn("Refuge " + r.getKey() + " waitingList size = " + wl);
        }
    ...
    private void updateDamage(ChangeSet changes) {
        for (HumanAttributes ha : humans.values()) {
            ...
            /*
                By Farshid Faraji to implement Refuge Bed Capacity on May 2020
    ...
    private void updateRefuges()
    {
        ...
        {
            //Logger.warn("Waiting list size = " + ((Deque<EntityID>)e.getValue()).size());
            ...
            for(EntityID civ: (Deque<EntityID>)e.getValue())
            {
                if(model.getEntity(civ) instanceof Human )
                {
                    ...
                    {
                        tempList.add(civ);
                        Logger.warn("Added to tempList " + civ + " hp = " + ((Human)model.getEntity(civ)).getHP() + " damage = " + ((Human)model.getEntity(civ)).getDamage());
                    }
                }
            }
            if(tempList.size() > 0) {
                int ws = ((Deque<EntityID>) e.getValue()).size();
                ((Deque<EntityID>) e.getValue()).removeAll(tempList);
                Logger.warn("removing temp list " + ws + " ==> " + ((Deque<EntityID>) e.getValue()).size());
            }
        }
            ...
            if(e instanceof Refuge) {
                while (((Refuge)e).getOccupiedBeds() < ((Refuge)e).getBedCapacity()) {
                    if(waitingList.get(e.getID()).size() > 0) {
                        ...
                        Logger.warn("updateRefuges : waiting list " + e.getID() + " removed " + removedid + " size = " + waitingList.get(e.getID()).size());
