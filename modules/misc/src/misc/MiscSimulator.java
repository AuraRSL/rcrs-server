...
public class MiscSimulator extends StandardSimulator implements GUIComponent {
    ...
    protected void processCommands(KSCommands c, ChangeSet changes) {
        ...
        for (Command com : c.getCommands()) {
            ...
            if (checkValidity(com)) {
                ...
                if (com instanceof AKUnload) {
                    Logger.warn("AkUnload ............. Command");
                    EntityID agentID = com.getAgentID();
                    ...
                    if (target != null) {
                        Entity pos = model.getEntity(((Human) agent).getPosition());
                        if (pos instanceof Refuge) {
                            waitingList.get(pos.getID()).add(target.getID());
                            Logger.warn(target + " added to waiting list with Damage " + target.getDamage());
                            refreshRefugeCapacity(pos, changes);
                        }
                    }
        ...
        updateRefugeWaitingList(changes);
    ...
    private void updateDamage(ChangeSet changes) {
        for (HumanAttributes ha : humans.values()) {
            ...
            // Damage is reducing but HP is fix
            //human will not die in refuge but take while to get damage to 0
            else {

                if (waitingList.get((h.getPosition(model)).getID()).contains(h.getID())) {
                    updateDamage(ha);
                    int hp = h.isHPDefined() ? h.getHP() : 0;
                    int damage = ha.getTotalDamage();
                    ...
                    changes.addChange(ha.getHuman(), ha.getHuman()
                            .getDamageProperty());
                    if (oldDamage > 0 && h.getDamage() <= 0 && ((Refuge) h.getPosition(model)).getOccupiedBeds() > 0) {
                        ((Refuge) h.getPosition(model)).setOccupiedBeds(((Refuge) h.getPosition(model)).getOccupiedBeds() - 1);
                        if (waitingList.get(h.getPosition()).size() > 0)
                            waitingList.get(h.getPosition()).remove();
                        changes.addChange(((Refuge) h.getPosition(model)), ((Refuge) h.getPosition(model)).getOccupiedBedsProperty());
                    }
                }
    ...
    private void refreshRefugeCapacity(Entity pos, ChangeSet changes) {
        if (!(pos instanceof Refuge))
            return;
        while (((Refuge) pos).getOccupiedBeds() < ((Refuge) pos).getBedCapacity() && waitingList.get(pos.getID()).size() > 0) {
            EntityID id = waitingList.get(pos.getID()).remove();
            ((Refuge) pos).setOccupiedBeds(((Refuge) pos).getOccupiedBeds() + 1);
            changes.addChange(((Refuge) pos), ((Refuge) pos).getOccupiedBedsProperty());
        }
    }

    private void updateRefugeWaitingList(ChangeSet changes)
    {
        for(StandardEntity e: model.getEntitiesOfType(StandardEntityURN.REFUGE)) {
            if(e instanceof Refuge) {
                while (((Refuge)e).getOccupiedBeds() < ((Refuge)e).getBedCapacity() && waitingList.get(e.getID()).size() > 0) {
                    waitingList.get(e.getID()).remove();
                    ((Refuge)e).setOccupiedBeds(((Refuge)e).getOccupiedBeds() + 1);
