...
public class DecoratorOverlay implements Overlay {
     ...
     *            The buildings to remove any custom decorator for.
     */
    public void clearBuildingDecorator(
            Collection<? extends GMLBuilding> buildings) {
        for (GMLBuilding next : buildings) {
    ...
    public void render(Graphics2D g, ScreenTransform transform) {
        ...
        for (Entry<GMLBuilding, BuildingDecorator> e : buildingDecorators
            ...
            if(((FilledShapeDecorator)e.getValue()).getBuildingColour().equals(new Color(0, 128, 0)))
            {
                ...
                g.setFont(new Font(g.getFont().getName(), Font.BOLD, g.getFont()
                        .getSize()));
                g.drawString(String.valueOf("B " + e.getKey().getBedCapacity()), x - 10, y);
                g.drawString(String.valueOf("R " + e.getKey().getRefillCapacity()), x - 10, y + 10);
                g = oldg;
            }
        }
